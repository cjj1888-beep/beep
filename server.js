...
async function initBrowser() {
    // [중요] 매 요청마다 브라우저를 새로 띄우지 않고, 하나를 계속 쓰거나
    // 디버깅을 위해 매번 새로 띄우되 닫지 않게 설정
    // 여기서는 매번 새로 띄우는 방식으로 변경 (충돌 방지)
...
app.post('/scrape', async (req, res) => {
    ...
    try {
        localBrowser = await initBrowser(); // 매번 새 브라우저
        page = await localBrowser.newPage();

        // 화면 큼직하게
        const pages = await localBrowser.pages();
        if (pages.length > 0) page = pages[0]; // 첫번째 탭 사용
        ...
        // [요청] 브라우저 안 닫음 (디버깅용)
        console.log("👀 디버깅을 위해 브라우저를 닫지 않습니다. (수동으로 닫아주세요)");
